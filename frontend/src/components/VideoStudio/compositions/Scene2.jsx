...
export function Scene2({ scene2Headline: headline, playerName, playerImage, heroSrc, stats, brandName, bgColor, accent, cardColor, cardAccent }) {
  ...
  const heroUrl = heroSrc?.startsWith('http') ? heroSrc : null
  const playerUrl = playerImage?.startsWith('http') ? playerImage : null
  ...
  return (
    <AbsoluteFill style={{ background: bgColor, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', left: 80, right: 80, bottom: 100, height: HEADLINE_RESERVE - 100, zIndex: 4, display: 'flex', alignItems: 'flex-end', fontFamily: '"Anek Malayalam", "Noto Sans Malayalam", system-ui, sans-serif', fontWeight: 700, fontSize: 64, lineHeight: 1.25, letterSpacing: '-0.005em', color: '#fff' }}>
        <div>
          {words.map((word, i) => {
